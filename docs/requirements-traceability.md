...
| Vendor-neutral guided installation and removal | passive components, policy-ledger extraction switch, project/global intent config, pinned-source option, stable-venv PowerShell/Bash bootstrap, owned-hook/MCP cleanup with durable-data preservation | installer policy/config/portability and setup/uninstall symmetry tests plus clean bootstrap smoke |
