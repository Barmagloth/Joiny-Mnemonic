...
HTTP or hooks is treated as a write unless the method is in the proven
read-only set. `--require-clean` is the final Stage 3 acceptance mode.
...
store calls, not queries. Unknown methods are never silently classified as
reads.
