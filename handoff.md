...
3. **Любой новый set-selector** сравнивать не с абстрактным top-k, а с
   настроенным session-diversity packing, на замороженном наборе; до
   этого — census конкретных ошибок комплектации. NB: census в основном
   вычислим из уже сохранённых артефактов — пер-вопросные строки всех
   прогонов содержат retrieved_ids, gold_sessions, coverage и ответы;
   «золото найдено, ответ провален» выделяется офлайн без LLM.
...
4. Extraction gate — **первый цикл пройден, выводы СУЖЕНЫ ревью
   пользователя (2026-07-17)**. Доказано: механизм гейта останавливает
