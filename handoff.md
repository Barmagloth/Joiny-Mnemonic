...
1b. ~~[CHECK MATERIAL]~~ — **закрыт как accuracy-эксперимент по
   ...
   Флаг --check-material остаётся в harness как измеренная опция.
   Продуктовая verify-поверхность (auditability, не точность) — открытый
   UX-вопрос, бенчем не мерится.
