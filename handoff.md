...
1. **Update-aware ось решена keyed-выжимкой (2026-07-16, stage 3 в
   ...
   НЕ сделано: preference-проверка keyed-промпта (кеш только KU;
   ~1ч прогрев + 40мин бенч) — обязательна до любого шипа.
1b. **[CHECK MATERIAL] секция в packing** (идея пользователя 2026-07-16):
   для verify-подобных запросов пакет несёт ограниченную секцию — exact
   sources + КОНФЛИКТУЮЩИЕ near-dup факты с датами (не разрешать молча,
   а предъявить читателю) + relevant failures/lessons + stale/precheck
   warnings. Атакует оба недостижимых для ingest класса: отравленное
   воздержание и «расплывчатый преемник». Провалившийся токен-детектор
   (0.30) здесь безопасен: ложное флагование стоит строк, не правды.
   Оговорки: жёсткий cap (бюджет пакета связан), решить когда включать
   (всегда vs verify-режим). Проверка: KU+abstention пробы, парно, БЕЗ
   пере-выжимки (сборка при упаковке из любого кеша).
