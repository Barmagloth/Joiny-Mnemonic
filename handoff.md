...
3. **Любой новый set-selector** сравнивать не с абстрактным top-k, а с
   ...
   **Census СДЕЛАН (2026-07-17, benchmarks/results/census.md)**: из 60
   провалов raw-500 — 90% (54) reader/synthesis (золото полностью в
   пакете), 5 packing (в пуле-64, но за бюджетной линией), 1 retrieval,
   0 утечек. Потолок селекторной работы = ~1pp; приоритет снят данными.
   Главный класс — последняя миля (ms 18 / temporal 16 / preference 12).
