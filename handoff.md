...
1. ~~Update-aware distillation~~ — **ячейка измерена и закрыта
   (2026-07-16, вторая секция distill-ab.md)**: детерминированная
   supersession по токен-перекрытию (порог 0.30, `--ingest distill-aware`)
   чинит ровно value-update класс (+2 из 78 KU), но in vivo даёт 170
   срабатываний на ~2 настоящих апдейта (повторяющиеся темы неотличимы
   от апдейтов токенами) — net −1, в пределах шума. Перепись 5 исходных
   KU-провалов: 1 value-update, 1 низко-оверлапный апдейт, 1 пропуск
   дистиллятора, 1 требует перечисления, 1 отравленное воздержание —
   т.е. update-aware ось детерминированно НЕ закрывается. Следующие
   формы по цене: answer-time recency-дисциплина для near-dup фактов →
   write-time LLM-реконсиляция (форма Letta/Mem0) → entity-slot keying.
   Ничто не шипится дефолтом, пока не побьёт замеренную планку.
