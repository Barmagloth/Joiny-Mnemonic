...
### Оценка готовности (2026-07-17, формулировка пользователя)

**Центральный сценарий НЕ доказан: обычный диалог → система сама надёжно
выделяет долговременную запись.** Работают: журнал, hooks, resume/MCP,
retrieval+reranker, provenance, snapshots, settlement, явные маркеры с
ручным подтверждением, перф и диагностика каналов — всё это
инфраструктура ВОКРУГ непроверенного ядра. Поставляемый экстрактор не
измерен; центральный acceptance пропущен; готовность была завышена.
Acceptance зафиксирован в TODO (шапка): шипуемый бэкенд проходит корпус
на held-out + живой e2e на реальном сторе без маркеров.
...
4. Extraction: **ПЕРЕКВАЛИФИЦИРОВАНО ревью 2026-07-17 — это был research
   ...
   а слово поехало за артефактом). ПОПРАВКА (2026-07-17, поймано
   пользователем): в объяснении и в тексте коммита a0981fd я написал,
   что nuextract «would likely fail hard / скорее всего провалился бы» —
   это домысел без единого измерения, к тому же мотивированный (задним
   числом оправдывал выбор по удобству). Единственные факты: bridge был
   меньшей работой; nuextract требует установки. Как поведёт себя
   nuextract-local на корпусе — НЕИЗВЕСТНО. Поправка дописана, история
   не переписывалась. Продукт-гейт = тот же корпус против
   nuextract-local, в очереди TODO#6; исход не предсказываем. Первый цикл пробы (выводы сужены ревью): Доказано: механизм гейта останавливает
